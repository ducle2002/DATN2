...
namespace Yootek.Application.Configuration.Tenant
{
    ...
    public class TenantSettingsAppService : YootekAppServiceBase, ITenantSettingsAppService
    {
        ...
        public async Task<DigitalManagementConfigDto> GetAllDigitalManagementConfig()
        {
            return new DigitalManagementConfigDto
            {

                ReflectConfig = await GetReflectConfig(),
            };
        }
        ...
        private async Task<ReflectConfigSettingDto> GetReflectConfig()
        {
            try
            {
                return new ReflectConfigSettingDto
                {
                    AddWork = await SettingManager.GetSettingValueForTenantAsync<bool>(
                        AppSettings.TenantManagement.ReflectConfig.AddWork,
                        AbpSession.TenantId.Value),
                    AddHandler = await SettingManager.GetSettingValueForTenantAsync<bool>(
                        AppSettings.TenantManagement.ReflectConfig.AddHandler,
                        AbpSession.TenantId.Value),
                    DisableOrganizationUnit = await SettingManager.GetSettingValueForTenantAsync<bool>(
                        AppSettings.TenantManagement.ReflectConfig.DisableOrganizationUnit,
                        AbpSession.TenantId.Value),
        ...
        public async Task UpdateDigitalManagementConfig(DigitalManagementConfigDto input)
        {
            await UpdateReflectConfig(input.ReflectConfig);
        }
        ...
        private async Task UpdateReflectConfig(ReflectConfigSettingDto input)
        {
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.TenantId.Value,
               AppSettings.TenantManagement.ReflectConfig.AddWork,
               input.AddWork.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.TenantId.Value,
               AppSettings.TenantManagement.ReflectConfig.AddHandler,
               input.AddHandler.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.TenantId.Value,
               AppSettings.TenantManagement.ReflectConfig.DisableOrganizationUnit,
               input.DisableOrganizationUnit.ToString());
        }
