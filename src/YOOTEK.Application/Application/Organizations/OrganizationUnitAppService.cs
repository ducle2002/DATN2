...
namespace Yootek.Service
{
    ...
    public class OrganizationUnitAppService : YootekAppServiceBase, IOrganizationUnitAppService
    {
        ...
        public async Task<DataResult> GetOrganizationUnitByUrbanId(long urbanId)
        {
            try
            {
                var urban = await _organizationUnitRepository.FirstOrDefaultAsync(urbanId);
                if (urban == null) return DataResult.ResultSuccess("");
                var ids = await _organizationUnitRepository.GetAll()
                    .Where(x => x.Code.StartsWith(urban.Code) 
                ...
                return DataResult.ResultSuccess(organizationUnits.Select(ou =>
                {
                    var a = ObjectMapper.Map<OrganizationUnitDto>(ou);
                    return a;
                }).ToList(), "");
            }
            catch (Exception e)
        ...
        public async Task<ListResultDto<OrganizationUnitDto>> GetOrganizationUnitsByUser()
        {
            try
            {
                if(IsGranted(IOCPermissionNames.Data_Admin))
                {
                    var organizationUnits = await _organizationUnitRepository.GetAll()
                        .Where(x => x.Type == APP_ORGANIZATION_TYPE.REPRESENTATIVE_NAME)
                        .ToListAsync();
                ...
                {
                    var ous = await (from ouUser in _userOrganizationUnitRepository.GetAll()
                    ...
                    var codes = ous.Select(x => x.Code).ToList();
                    var ids = ous.Select(x => x.Id).ToList();
                    var organizationUnits = await _organizationUnitRepository.GetAll()
                        .Where(x => x.Type == APP_ORGANIZATION_TYPE.REPRESENTATIVE_NAME)
                        .Where(x => codes.Any(code => x.Code.StartsWith(code)))
                        .Distinct()
                        .ToListAsync();
                    ...
                    var allCodes = organizationUnits.Select(x => x.Code).Distinct().ToList();

                    var result = organizationUnits.Select(ou =>
                    {
                        var organizationUnitDto = ObjectMapper.Map<OrganizationUnitDto>(ou);
                        if (allCodes.Where(y => ou.Code.StartsWith(y)).Count() == 1) organizationUnitDto.ParentId = null;
                        organizationUnitDto.MemberCount = organizationUnitMemberCounts.ContainsKey(ou.Id) ? organizationUnitMemberCounts[ou.Id] : 0;
                        return organizationUnitDto;
                    }).ToList();
        ...
        public async Task<DataResult> GetAllMemberByOrganizationUnit(long id)
        {
            try
            {
                var org = await _organizationUnitRepository.FirstOrDefaultAsync(id);
                if (org == null) return DataResult.ResultSuccess("");
                var ids = await _organizationUnitRepository.GetAll()
                ...
                var result = await query.Where(x => ids.Contains(x.OrganizationUnitId)).Distinct().ToListAsync();
        ...
        public async Task<PagedResultDto<object>> GetOrganizationUnitToDepartment(GetOrganizationUnitToDepartment input)
        {
            var query = from ouUser in _userOrganizationUnitRepository.GetAll()
                        join urb in _organizationUnitRepository.GetAll() on ouUser.OrganizationUnitId equals urb.Id
                        where ouUser.UserId == AbpSession.UserId
            ...
            IQueryable<object> filteredQuery = query;

            if (input.Type != null)
            {
                filteredQuery = query.Where(x => x.Type == input.Type || x.Types.Any(t => input.Type.Value == t));
            }
        ...
        public async Task<OrganizationUnitDto> CreateOrganizationUnit(CreateOrganizationUnitInput input)
        {
            try
            {
                ...
                OrganizationUnit? orgUnit = await _organizationUnitRepository.FirstOrDefaultAsync(x => x.ProjectCode == input.ProjectCode && x.Type == input.Type);
                if (orgUnit != null) throw new UserFriendlyException(409, "OrganizationUnit is exist");
        ...
        {
            var organizationUnit = await _organizationUnitRepository.GetAsync(input.Id) ?? throw new UserFriendlyException("OrganizationUnit not found!");
            OrganizationUnit? listOrgUnit = await _organizationUnitRepository.FirstOrDefaultAsync(x =>
                x.Id != input.Id && x.ProjectCode == input.ProjectCode && x.Type == input.Type && x.ParentId != input.Id && input.Types.Contains(x.Type));
            if (listOrgUnit != null)
            {
