...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class PhieuXuatKhoAppService : YootekAppServiceBase, IPhieuXuatKhoAppService
    {
...
;
                List<PhieuXuatKhoDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortPhieuXuatKho.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
