...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class DonViTaiSanAppService : YootekAppServiceBase, IDonViTaiSanAppService
    {
...
x.Code.ToLower().Contains(input.Keyword.ToLower()) || x.Title.ToLower().Contains(input.Keyword.ToLower()) || x.Description.ToLower().Contains(input.Keyword.ToLower()));
                List<DonViTaiSanDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortDonViTaiSan.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
