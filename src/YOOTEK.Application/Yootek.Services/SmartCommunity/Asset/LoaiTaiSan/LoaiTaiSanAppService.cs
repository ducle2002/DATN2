...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class LoaiTaiSanAppService : YootekAppServiceBase, ILoaiTaiSanAppService
    {
...
x.Code.ToLower().Contains(input.Keyword.ToLower()) || x.Title.ToLower().Contains(input.Keyword.ToLower()) || x.Description.ToLower().Contains(input.Keyword.ToLower()));
                List<LoaiTaiSanDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortLoaiTaiSan.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
