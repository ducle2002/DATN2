...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class NhaSanXuatAppService : YootekAppServiceBase, INhaSanXuatAppService
    {
...
x.Code.ToLower().Contains(input.Keyword.ToLower()) || x.Title.ToLower().Contains(input.Keyword.ToLower()) || x.Description.ToLower().Contains(input.Keyword.ToLower()) || x.Address.ToLower().Contains(input.Keyword.ToLower()) || x.Phone.ToLower().Contains(input.Keyword.ToLower()));
                List<NhaSanXuatDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortNhaSanXuat.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
