...
IRepository<TaiSanChiTiet, long> taiSanChiTietRepository, IRepository<BlockTower, long> blockTowersRepository,
            ...
            IRepository<Floor, long> floorsRepository, IRepository<NhomTaiSan, long> nhomTaiSanRepository, IRepository<MaHeThong, long> maHeThongRepository)
        {
        ...
        public async Task<DataResult> GetAllAsync(GetAllNhatKyVanHanhInputDto input)
        {
            try
            {
                DateTime ngaySuaChuaFrom = new DateTime(), ngaySuaChuaTo = new DateTime();
                if (input.NgaySuaChuaFrom.HasValue)
                {
                    ngaySuaChuaFrom = new DateTime(input.NgaySuaChuaFrom.Value.Year, input.NgaySuaChuaFrom.Value.Month, input.NgaySuaChuaFrom.Value.Day, 0, 0, 0);
                }
                if (input.NgaySuaChuaTo.HasValue)
                {
                    ngaySuaChuaTo = new DateTime(input.NgaySuaChuaTo.Value.Year, input.NgaySuaChuaTo.Value.Month, input.NgaySuaChuaTo.Value.Day, 23, 59, 59);
                }
                DateTime ngayCheckListFrom = new DateTime(), ngayCheckListTo = new DateTime();
                if (input.NgayCheckListFrom.HasValue)
                {
                    ngayCheckListFrom = new DateTime(input.NgayCheckListFrom.Value.Year, input.NgayCheckListFrom.Value.Month, input.NgayCheckListFrom.Value.Day, 0, 0, 0);
                }
                if (input.NgayCheckListTo.HasValue)
                {
                    ngayCheckListTo = new DateTime(input.NgayCheckListTo.Value.Year, input.NgayCheckListTo.Value.Month, input.NgayCheckListTo.Value.Day, 23, 59, 59);
                }
                ...
                .WhereIf(input.NgaySuaChuaFrom.HasValue, u => u.NgaySuaChua.HasValue&&u.NgaySuaChua >= ngaySuaChuaFrom)
                .WhereIf(input.NgaySuaChuaTo.HasValue, u => u.NgaySuaChua.HasValue&&u.NgaySuaChua <= ngaySuaChuaTo)
                .WhereIf(input.NgayCheckListFrom.HasValue, u => u.NgayCheckList.HasValue&&u.NgayCheckList >= ngayCheckListFrom)
                .WhereIf(input.NgayCheckListTo.HasValue, u => u.NgayCheckList.HasValue&&u.NgayCheckList <= ngayCheckListTo)
                ;
                List<NhatKyVanHanhDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortNhatKyVanHanh.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
        ...
        public async Task<DataResult> GetById(long id)
        {
            try
            {
                ...
                if (taisan != null)
                {
                    ...
                    data.MaHeThongId=taisan.MaHeThongId;
                    data.NhomTaiSanId=taisan.NhomTaiSanId;
                    data.NhomTaiSanText = _nhomTaiSanRepository.GetAll().Where(x => x.Id == taisan.NhomTaiSanId).Select(x => x.Title).FirstOrDefault();
                    data.MaHeThongText = _maHeThongRepository.GetAll().Where(x => x.Id == taisan.MaHeThongId).Select(x => x.Title).FirstOrDefault();
                }
