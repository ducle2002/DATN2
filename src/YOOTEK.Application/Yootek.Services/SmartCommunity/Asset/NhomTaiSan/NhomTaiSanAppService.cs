...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class NhomTaiSanAppService : YootekAppServiceBase, INhomTaiSanAppService
    {
        ...
        public async Task<DataResult> GetAllAsync(GetAllNhomTaiSanInputDto input)
        {
            try
            {
                ...
                List<NhomTaiSanDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortNhomTaiSan.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
