...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class TaiSanAppService : YootekAppServiceBase, ITaiSanAppService
    {
...
x.Code.ToLower().Contains(input.Keyword.ToLower()) || x.Title.ToLower().Contains(input.Keyword.ToLower()) || x.Description.ToLower().Contains(input.Keyword.ToLower()))
    ...
    .WhereIf(input.IsHasDonViTinh==true, x => x.DonViTinhId.HasValue)
;
                List<TaiSanDto> result = await query.ApplySort(input.OrderBy??FieldSortTaiSan.ID, input.OrderBy.HasValue ? input.SortBy : SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
