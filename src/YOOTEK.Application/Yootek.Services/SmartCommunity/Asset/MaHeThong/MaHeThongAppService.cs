...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class MaHeThongAppService : YootekAppServiceBase, IMaHeThongAppService
    {
...
x.Code.ToLower().Contains(input.Keyword.ToLower()) || x.Title.ToLower().Contains(input.Keyword.ToLower()) || x.Description.ToLower().Contains(input.Keyword.ToLower()));
                ...
                List<MaHeThongDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortMaHeThong.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
