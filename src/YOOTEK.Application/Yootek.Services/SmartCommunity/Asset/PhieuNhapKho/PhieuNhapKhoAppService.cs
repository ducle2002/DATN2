...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class PhieuNhapKhoAppService : YootekAppServiceBase, IPhieuNhapKhoAppService
    {
...
;
                List<PhieuNhapKhoDto> result = await query.ApplySort(input.OrderBy, (SortBy)input.SortBy).ApplySort(FieldSortPhieuNhapKho.ID, SortBy.DESC).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
