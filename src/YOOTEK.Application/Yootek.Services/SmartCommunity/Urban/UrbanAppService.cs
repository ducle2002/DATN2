...
namespace Yootek.Services
{
    ...
    [AbpAuthorize]
    public class UrbanAppService : YootekAppServiceBase, IUrbanAppService
    {
        ...
        public async Task<DataResult> GetAllUrbansTreeAsync(GetAllUrbansInput input)
        {
            try
            {
                List<AppOrganizationUnit> urbans = _organizationUnitRepository.GetAllList(x => x.Type == APP_ORGANIZATION_TYPE.URBAN && x.ParentId.HasValue);
                ...
                IQueryable<UrbanTreeGetAllDto> query = (from dt in _organizationUnitRepository.GetAll()
                                                        join bd in _organizationUnitRepository.GetAll()
                                                        on dt.Id equals bd.ParentId into dt_bd
                                                        from bd in dt_bd.DefaultIfEmpty()
                                                        join ap in _apartmentRepository.GetAll() on bd.Id equals ap.BuildingId into bd_ap
                                                        ...
                                                        select new UrbanTreeGetAllDto()
                                                        {
                                                            ...
                                                            BuildingIds = (from b in _organizationUnitRepository.GetAll()
                                                                           where buildingIds.Contains(b.Id) && b.ParentId == g.Key.Id
                                                                           select b.Id).ToArray(),
                                                        })
                             .Where(x => urbanIds.Contains(x.Id))
                ...
                {
                    var buildingInfos = (from b in _organizationUnitRepository.GetAll()
                                         where item.BuildingIds.Contains(b.Id)
                                         orderby b.CreationTime
                                         select new BuildingDto
                                         {
                                             BuildingId = b.Id,
                                             NameBuilding = b.DisplayName,
                                             NumberApartments = _apartmentRepository.GetAll().Count(a => a.BuildingId == b.Id)
                                         }).ToList();

                    item.Buildings = buildingInfos;
                }
