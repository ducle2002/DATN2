...
namespace Yootek.Services.ExportData
{
    public interface IWorkExcelExporter
    {
        ...
        FileDto ExportWorkPlanOrganizationUnitToFile(WorkPlanByOrganizationDto data, DateTime month);
        FileDto ExportWorkPlanUserToFile(List<WorkPlanByUserDto> data, DateTime month);
    }
    public class WorkExcelExporter : NpoiExcelExporterBase, IWorkExcelExporter
    {
            ...
            return CreateExcelPackage(
                ...
                excelPackage =>
                {
                    ...
                    foreach (var item in items)
                    {
                        ...
                        CreateClsExcel(sheet, rowIndex, columnIndex + 2, "Logtime chưa đạt", HorizontalAlignment.Center, VerticalAlignment.Center, true, true, false, 11);
                        ...
                        foreach (var workDetail in item.ListWorkDetails)
                        {
                            ...
                            CreateClsExcel(sheet, rowIndex, columnIndex + 2, item.WorkLogTimes.Count(x => x.Status > 0 && x.QualityWork == EWorkQuality.NOTGOOD && x.WorkDetailId == workDetail.Id).ToString(), HorizontalAlignment.Center, VerticalAlignment.Center, false, true, false, 11);
        ...
        public FileDto ExportWorkPlanOrganizationUnitToFile(WorkPlanByOrganizationDto data, DateTime month)
        {

            return CreateWorkPlanFile(data.WorkPlans, data.OrganizationName, month);

        }

        public FileDto ExportWorkPlanUserToFile(List<WorkPlanByUserDto> data, DateTime month)
        {

            return CreateWorkPlanFile(ObjectMapper.Map<List<WorkPlanDetailDto>>(data), "nhanvien", month);
        }

        private FileDto CreateWorkPlanFile(List<WorkPlanDetailDto> workPlans, string fileName, DateTime month)
        {
            return CreateExcelPackage(
           $"Kehoachcongviec_{fileName}.xlsx",
           excelPackage =>
           {
              var sheet = excelPackage.CreateSheet("WorkPlan");
              ...
              var countWork = workPlans.Count() - 1;

              IRow aptRow1 = sheet.CreateRow(0);
              IRow aptRow2 = sheet.CreateRow(1);
              CreateCellMergeHeader(sheet, aptRow1, "Tên công việc", 0, 1, 0, 0);
              CreateCellMergeHeader(sheet, aptRow1, "Danh mục công việc chi tiết", 0, 1, 1, 1);
              CreateCellMergeHeader(sheet, aptRow1, "Tần suất", 0, 1, 2, 2);

              // Create day in month 
              var countDay = month.TotalDaysInMonth();
              for (var i = 1; i <= countDay; i++)
              {
                  var day = new DateTime(month.Year, month.Month, i);
                  CreateCellHeader(sheet, aptRow1, GetDayOfWeek(day.DayOfWeek), i + 2);
                  CreateCellHeader(sheet, aptRow2, day.Day + "", i + 2);
                  foreach (var item in workPlans)
                  {
                      if (item.ListWorkDetail == null) continue;
                      foreach (var dt in item.ListWorkDetail)
                      {
                          var check = false;
                          var options = dt.FrequencyOption != null ? JsonConvert.DeserializeObject<List<int>>(dt.FrequencyOption) : new List<int>();
                          switch (dt.Frequency)
                          {
                              case 1:
                                  check = true;
                                  break;
                              case 2:
                                  if (options.Contains(GetNumberDayOfWeek(day))) check = true;
                                  break;
                              case 3:
                                  if (options.Contains(day.Day)) check = true;
                                  break;
                              case 4:
                                  if (options.Contains(day.Month) && dt.DayOfMonth == day.Day) check = true;
                                  break;
                              default:
                                  check = false;
                                  break;
                          ...
                          if (dt.Checks == null) dt.Checks = new List<bool>();
                          dt.Checks.Add(check);
                      }
                  }
              ...
              int workRowIndex = 1;
              foreach (var item in workPlans.Select((value, index) => { return new { Value = value, Index = index }; }))
              {
                  IRow row;
                  if (item.Value.ListWorkDetail == null) continue;
                  if (item.Value.ListWorkDetail.Count() > 1)
                  {
                      CreateCellMerge(sheet, item.Value.Title, workRowIndex + 1, workRowIndex + item.Value.ListWorkDetail.Count(), 0, 0, headFontNormal);
                      row = sheet.GetRow(workRowIndex + 1);
                  }
                  else
                  {
                      row = sheet.CreateRow(workRowIndex + 1);
                      CreateCellString(row, 0, item.Value.Title);
                  }
                  foreach (var dt in item.Value.ListWorkDetail.Select((value, index) => { return new { Value = value, Index = index }; }))
                  {
                      IRow rowDetail;
                      if (dt.Index == 0)
                      {
                      ...
                      {
                          rowDetail = sheet.CreateRow(workRowIndex + 1 + dt.Index);
                      }

                      CreateCellString(rowDetail, 1, dt.Value.Name);
                      CreateCellString(rowDetail, 2, $"{dt.Value.FrequencyTimes} lần/ {GetTextFrequency(dt.Value.Frequency)}");
                      if (dt.Value.Checks == null) continue;
                      foreach (var check in dt.Value.Checks.Select((value, index) => { return new { Value = value, Index = index }; }))
                      {
                          if (check.Value) CreateCellString(rowDetail, 3 + check.Index, "X");
                      }
                  }
                  workRowIndex = workRowIndex + item.Value.ListWorkDetail.Count();
              }
           });

        }

        private string GetTextFrequency(int? dt)
        {
            if (dt == 1) return "ngày";
            else if (dt == 2) return "tuần";
            else if (dt == 3) return "tháng";
            else if (dt == 4) return "năm";
            return "";
        }
        ...
        protected int GetNumberDayOfWeek(DateTime time)
        {
            if (time.DayOfWeek == DayOfWeek.Sunday) return 7;
