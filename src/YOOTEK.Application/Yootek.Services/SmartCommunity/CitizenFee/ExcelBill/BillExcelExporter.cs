        ...
        public FileDto ExportAllDetailUserBills(List<ApartmentPaymentBillDetailDto> data, List<DateTime> listMonths)
        {
            try
            {
               ...
               excelPackage =>
               {
                   ...
                   CreateCellMergeHeader(sheet, aptRow1, "Trạng thái mặt bằng", 0, 2, 4, 4);
                   CreateCellMergeHeader(sheet, aptRow1, "Phí quản lý dịch vụ (từ thời điểm bàn giao đến ngày)", 0, 0, 5, 10);
                   CreateCellMergeHeader(sheet, aptRow2, "Nợ đầu tháng", 1, 2, 5, 5);
                   CreateCellMergeHeader(sheet, aptRow2, "Phải thu trong tháng", 1, 2, 6, 6);
                   CreateCellMergeHeader(sheet, aptRow2, "Tổng phải thu", 1, 2, 7, 7);
                   CreateCellMergeHeader(sheet, aptRow2, "Đã thu trong tháng", 1, 2, 8, 8);
                   CreateCellMergeHeader(sheet, aptRow2, "Trả trước", 1, 2, 9, 9);
                   CreateCellMergeHeader(sheet, aptRow2, "Số dư đến cuối tháng", 1, 2, 10, 10);

                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí quản lý dịch vụ", listMonths, 11, 0);
                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí điện", listMonths, 17, 1);
                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí nước", listMonths, 23, 2);
                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí gửi xe tháng", listMonths, 29, 3);
                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí gửi xe ô tô", listMonths, 35, 4);
                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí gửi xe máy/ Xe máy điện/ Xe đạp điện", listMonths, 41, 5);
                   CreateHeaderMhome(sheet, aptRow1, aptRow2, aptRow3, "Phí gửi xe đạp", listMonths, 47, 6);
                   ...
                   foreach (var item in data)
                   {
                       ...
                       CreateCellString(row, 4, item.ApartmentStatusName);

                       CreateCellCost(row, 5, item.ManagementDebtAll);
                       CreateCellCost(row, 6, item.ManagementCostAll);
                       CreateCellCost(row, 7, item.ManagementTotalAll);
                       CreateCellCost(row, 8, item.ManagementPaidAll);
                       CreateCellCost(row, 9, item.ManagementPrepayAll);
                       CreateCellCost(row, 10, item.ManagementBalanceAll);

                       CreateCellCost(row, 11, item.ManagementDebt);
                       CreateCellCost(row, 12, item.ManagementCost);
                       CreateCellCost(row, 13, item.ManagementTotal);
                       CreateCellCost(row, 15 + countMonth, item.ManagementPrepay);
                       CreateCellCost(row, 16 + countMonth, item.ManagementBalance);

                       CreateCellCost(row, 17 + countMonth, item.ElectricDebt);
                       CreateCellCost(row, 18 + countMonth, item.ElectricCost);
                       CreateCellCost(row, 19 + countMonth, item.ElectricTotal);
                       CreateCellCost(row, 21 + 2 * countMonth, item.ElectricPrepay);
                       CreateCellCost(row, 22 + 2 * countMonth, item.ElectricBalance);

                       CreateCellCost(row, 23 + 2 * countMonth, item.WaterDebt);
                       CreateCellCost(row, 24 + 2 * countMonth, item.WaterCost);
                       CreateCellCost(row, 25 + 2 * countMonth, item.WaterTotal);
                       CreateCellCost(row, 27 + 3 * countMonth, item.WaterPrepay);
                       CreateCellCost(row, 28 + 3 * countMonth, item.WaterBalance);

                       CreateCellCost(row, 29 + 3 * countMonth, item.ParkingDebt);
                       CreateCellCost(row, 30 + 3 * countMonth, item.ParkingCost);
                       CreateCellCost(row, 31 + 3 * countMonth, item.ParkingTotal);
                       CreateCellCost(row, 33 + 4 * countMonth, item.ParkingPrepay);
                       CreateCellCost(row, 34 + 4 * countMonth, item.ParkingBalance);

                       CreateCellCost(row, 35 + 4 * countMonth, item.CarDebt);
                       CreateCellCost(row, 36 + 4 * countMonth, item.CarCost);
                       CreateCellCost(row, 37 + 4 * countMonth, item.CarTotal);
                       CreateCellCost(row, 39 + 5 * countMonth, item.CarPrepay);
                       CreateCellCost(row, 40 + 5 * countMonth, item.CarBalance);

                       CreateCellCost(row, 41 + 5 * countMonth, item.MotorDebt);
                       CreateCellCost(row, 42 + 5 * countMonth, item.MotorCost);
                       CreateCellCost(row, 43 + 5 * countMonth, item.MotorTotal);
                       CreateCellCost(row, 45 + 6 * countMonth, item.MotorPrepay);
                       CreateCellCost(row, 46 + 6 * countMonth, item.MotorBalance);

                       CreateCellCost(row, 47 + 6 * countMonth, item.BikeDebt);
                       CreateCellCost(row, 48 + 6 * countMonth, item.BikeCost);
                       CreateCellCost(row, 49 + 6 * countMonth, item.BikeTotal);
                       CreateCellCost(row, 51 + 7 * countMonth, item.BikePrepay);
                       CreateCellCost(row, 52 + 7 * countMonth, item.BikeBalance);

                       for (var i = 0; i <= countMonth; i++)
                       {
                           CreateCellCost(row, 14 + i, item.ManagementCellPaidMonths[i]?.TotalPaid ?? 0);
                           CreateCellCost(row, 20 + i + countMonth, item.ElectricCellPaidMonths[i]?.TotalPaid ?? 0);
                           CreateCellCost(row, 26 + i + 2 * countMonth, item.WaterCellPaidMonths[i]?.TotalPaid ?? 0);
                           CreateCellCost(row, 32 + i + 3 * countMonth, item.ParkingCellPaidMonths[i]?.TotalPaid ?? 0);
                           CreateCellCost(row, 38 + i + 4 * countMonth, item.CarCellPaidMonths[i]?.TotalPaid ?? 0);
                           CreateCellCost(row, 44 + i + 5 * countMonth, item.MotorCellPaidMonths[i]?.TotalPaid ?? 0);
                           CreateCellCost(row, 50 + i + 6 * countMonth, item.BikeCellPaidMonths[i]?.TotalPaid ?? 0);
                       }
