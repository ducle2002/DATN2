        ...
        public async Task<object> GetAllUserBillPayments(GetAllAdminUserBillPaymentDto input)
        {
            try
            {
                ...
                foreach (var bill in result)
                {
                    ...
                    if (bill.CustomerName.IsNullOrEmpty())
                    {
                        var properties = bill.BillList?.Count > 0 ? _userBillRepo.GetAll().Where(x => bill.BillList.Select(y => y.Id).Contains(x.Id)).Select(x => x.Properties).ToList() : null;
                        if(properties != null)
                        {
                            try
                            {
                                ...
                                {
                                    var customerName = JsonConvert.DeserializeObject<dynamic>(pro).customerName;
