...
namespace Yootek.Services
{
    ...
    public class AdminCitizenReflectAppService : YootekAppServiceBase, IAdminCitizenReflectAppService
    {
        ...
        private readonly ICitizenReflectExcelExporter _feedbackExcelExporter;
        ...
        public AdminCitizenReflectAppService(
            ...
            ICitizenReflectExcelExporter feedbackExcelExporter
        )
        {
            ...
            _feedbackExcelExporter = feedbackExcelExporter;
        }
        ...
        protected IQueryable<CitizenReflectDto> QueryDataCitizenReflect(GetCitizenReflectInput input)
        {
            ...
            IQueryable<CitizenReflectDto> query = (from fb in _citizenReflectRepos.GetAll()
                                                   ...
                                                   select new CitizenReflectDto()
                                                   {
                                                       ...
                                                       ListSupervisorIds = fb.ListSupervisorIds,
                ...
                .WhereIf(input.HandleOrganizationUnitId.HasValue, x => x.HandleOrganizationUnitId == input.HandleOrganizationUnitId)
                .WhereIf(input.SupervisorId.HasValue, x => x.ListSupervisorIds != null && x.ListSupervisorIds.Contains(input.SupervisorId.Value))
                .WhereIf(input.HandleUserId.HasValue, x => x.ListHandleUserIds != null && x.ListHandleUserIds.Contains(input.HandleUserId.Value))
            ...
            try
            {
                ...
                if (query != null)
                {
                    ...
                    foreach (var item in result)
                    {
                        ...
                        item.SupervisorsName = await GetUsersOrganizationNameAsync(item.ListSupervisorIds);
                    }
        ...
        public async Task<DataResult> GetCitizenReflectById(long id)
        {
            try
            {
                ...
                citizenReflectDto.SupervisorsName = await GetUsersOrganizationNameAsync(citizenReflect.ListSupervisorIds);
        ...
        public async Task<object> UpdateStateCitizenReflect(UpdateStateReflectInput input)
        {
            try
            {
                ...
                if (updateData != null)
                {
                    ...
                    if (input.State == STATE_FEEDBACK.ADMIN_CONFIRMED)
                    {
                        updateData.HandleUserId = AbpSession.UserId ?? 0;
                    }
        ...
        public async Task<object> ExportFeedbacksToExcel(ReflectExportExcelInput input)
        {
            try
            {
                var orIds = (from ou in _organizationRepos.GetAll()
                             join uou in _userOrganizationRepos.GetAll() on ou.ParentId equals uou.OrganizationUnitId into tb_uou
                             from uou in tb_uou
                             select new
                             {
                    ...
                    .Where(x => x.UserId == AbpSession.UserId && x.Type == APP_ORGANIZATION_TYPE.FEEDBACK)
                    .Select(x => x.OrganizationUnitId).ToList();

                var query = (from fb in _citizenReflectRepos.GetAll()
                             select new CitizenReflectDto()
                             {
                                 CreatorUserId = fb.CreatorUserId,
                                 CreationTime = fb.CreationTime,
                                 Name = fb.Name,
                                 FullName = fb.NameFeeder,
                                 BuildingName = _organizationRepos.GetAll().Where(o => o.Id == fb.BuildingId)
                                                             .Select(b => b.DisplayName).FirstOrDefault(),
                                 UrbanName = _organizationRepos.GetAll().Where(o => o.Id == fb.UrbanId)
                                                             .Select(b => b.DisplayName).FirstOrDefault(),
                                 ApartmentCode = fb.ApartmentCode,
                                 Data = fb.Data,
                                 OrganizationUnitName = _organizationRepos.GetAll().Where(o => o.Id == fb.OrganizationUnitId)
                                                             .Select(b => b.DisplayName).FirstOrDefault(),
                                 HandlerName = _userRepos.GetAll().Where(u => u.Id == fb.HandleUserId).Select(u => u.FullName).FirstOrDefault(),
                                 State = fb.State,
                                 StateName = GetStateValue((STATE_FEEDBACK?)fb.State),
                                 OrganizationUnitId = fb.OrganizationUnitId,
                                 FinishTime = fb.FinishTime,
                                 LastModificationTime = fb.LastModificationTime,
                             ...
                             .Where(x => orIds.Contains(x.OrganizationUnitId.Value))
                             .ToList();

                var result = _feedbackExcelExporter.ExportToFile(query);
        ...
        private static string GetStateValue(STATE_FEEDBACK? state)
        {
            switch (state)
            {
                case STATE_FEEDBACK.PENDING: return "Phản ánh mới";
                case STATE_FEEDBACK.HANDLING:
                case STATE_FEEDBACK.DECLINED:
                    return "Đang xử lý";
                case STATE_FEEDBACK.USER_CONFIRMED:
                case STATE_FEEDBACK.ADMIN_CONFIRMED: return "Hoàn thành";
                case STATE_FEEDBACK.USER_RATE_FEEDBACK: return "Đã đánh giá";
                default: return "Phản ánh chờ xác nhận";
            }
        }
        #endregion
    }
