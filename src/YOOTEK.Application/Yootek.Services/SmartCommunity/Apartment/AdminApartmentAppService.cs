...
namespace Yootek.Services
{
    ...
    public class AdminApartmentAppService : YootekAppServiceBase, IAdminApartmentAppService
    {
        ...
        private readonly IRepository<District, string> _districRepo;
        private readonly IRepository<Ward, string> _wardRepo;
        ...
        public AdminApartmentAppService(
            ...
            IRepository<Province, string> provinceRepo,
            IRepository<District, string> districRepo,
            IRepository<Ward, string> wardRepo
                    ...
                    List<ApartmentExportDto> apartments =
                        (from apartment in _apartmentRepository.GetAll()
                         select (new ApartmentExportDto()
                         {
                             ...
                             ProvinceName = _provinceRepo.GetAll().Where(x => x.Code == apartment.ProvinceCode).Select(x => x.FullName).FirstOrDefault(),
                             DistrictName = _districRepo.GetAll().Where(x => x.Code == apartment.DistrictCode).Select(x => x.FullName).FirstOrDefault(),
                             WardName = _wardRepo.GetAll().Where(x => x.Code == apartment.WardCode).Select(x => x.FullName).FirstOrDefault(),
